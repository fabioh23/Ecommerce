...
class Cliente(models.Model):
    ...
    id_sessao = models.CharField(max_length=200, blank=True)
    usuario = models.OneToOneField(User, null=True, blank=True, on_delete=models.CASCADE)

    def __str__(self):
        return f"Nome: {str(self.nome)}, Email: {str(self.email)}"
...
class Tipo(models.Model): #(Camisa, Camiseta, Bermuda, Calça)
    nome = models.CharField(max_length=200, null=True, blank=True)
    slug = models.CharField(max_length=200, null=True, blank=True)
...
class Produto(models.Model):
    ...
    def __str__(self):
        return f"Nome: {str(self.nome)} / Categoria: {str(self.categoria)} / Tipo: {str(self.tipo)} / Preco: {str(self.preco)}"
...
class ItemEstoque(models.Model):
    ...
    def __str__(self):
        return (f"Nome: {str(self.produto.nome)}, Cor: {str(self.cor.nome)}, Tamanho: {str(self.tamanho)}, Quantidade: {str(self.quantidade)}, "
                f"Categoria: {str(self.produto.categoria.nome)}, Tipo: {str(self.produto.tipo.nome)}")
...
class Endereco(models.Model):
    ...
    def __str__(self):
        return f" {str(self.cliente.email)} - {self.rua} - {self.cidade} - {self.estado}"
...
class Pedido(models.Model):
    ...
    def __str__(self):
        return f"Cliente: {str(self.cliente.email)} - id_pedido {str(self.id)} - Finalizado: {str(self.finalizado)}"
...
class ItensPedido(models.Model):
    ...
    def __str__(self):
        return (f"Id pedido {str(self.pedido.id)} - Produto {str(self.item_estoque.produto)} - {str(self.item_estoque.tamanho)} - "
                f"{str(self.item_estoque.cor.nome)} - {str(self.quantidade)}")
...
class Pagamento(models.Model):
    ...
    def __str__(self):
        return f"{self.pedido.cliente.email} - Aprovado: {self.aprovado}"